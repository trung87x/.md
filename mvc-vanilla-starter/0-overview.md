...
export async function init(host, model, ctx) {
  ...
  const p = new Promise((res) =>
    setTimeout(() => res({ score: 100, id: model.userId }), 1500)
  );
...
export async function init(host, model, ctx) {
  ...
  form.addEventListener("submit", (e) => {
    e.preventDefault();
    ctx.navigate("users", {
      query: { q: qInput.value || "", page: 1, pageSize: model.pageSize },
    });
  });
  ...
  const totalPages = Math.max(
    1,
    Math.ceil((model.total || 0) / model.pageSize)
  );
  ...
  const toPage = (p) =>
    ctx.navigate("users", {
      query: { q: model.q || "", page: p, pageSize: model.pageSize },
    });
...
export async function init(host, model, ctx) {
  ...
  host.querySelector("#title").textContent = ok
    ? "Payment Success"
    : "Payment Failed";
